...
import { buildTasks, header, releaseVersion, repoRoot, sourceTasks } from './scriptUtils'
...
function generateTypesFile(taskPath: string): void {
    console.log(`Generating types at ${taskPath}...`)
    const taskJsonPath = path.join(sourceTasks, taskPath, taskJson)
    const task = JSON.parse(fs.readFileSync(taskJsonPath, 'utf-8'))
...
`
    try {
        content += generateInputTypes(task)
        content += '\n' + generateMessageTypes(task)
    } catch (err) {
        throw new Error(`Error generating types for task ${taskPath}: ${err}`)
    }

    const inputTypesPath = path.join(sourceTasks, taskPath, 'types.gen.ts')
    fs.writeFileSync(inputTypesPath, content)
}

function generateInputTypes(task: Record<string, any>) {
    let content = `${header}
    
/* NOTE! this file is auto-generated by generateResources.ts, do not edit it by hand. */

export type ${task.name}Input = {`

    for (const i of task.inputs) {
        content += `\n    ${i.name}: `
        switch (i.type) {
            case 'connectedService:AWS':
            case 'filePath':
            case 'multiLine':
            case 'string':
                content += `string`
            ...
            case 'radio':
            case 'pickList':
                if (i.name === 'regionName' || i.label === 'AWS Region') {
                    // Special case where AWS regions are populated and should not be typed.
                    content += 'string'
                } else {
                    content += `${Object.keys(i.options).map(o => `'${o}'`).join(' | ')}`
                }
                break
            default:
                throw new Error(`Unknown task input type: '${i.type}'`)
        }
    }
    content += '\n}\n'
...
function generateMessageTypes(task: Record<string, any>) {
    if (task.messages.length === 0) {
        return '\n'
    }
    let content = `export type ${task.name}Messages = \n`
    for (const m of Object.keys(task.messages)) {
        content += `    | '${m}'\n`
    }
